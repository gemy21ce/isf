 ...
 * 
 */
class Categories extends AdminGenericController {
    ...
    function home() {
        $data['main_content'] = 'frontend/superjudge/category/home_view';
    ...
    function add() {
        ...
        $groups->get();
        $data['groups'] = $groups;
        $data['main_content'] = 'frontend/superjudge/category/add_category';
        $this->load->view('frontend/includes/template', $data);
    }
...
//            return $this->showGoodStatusPage("قائم جديدة '" . $this->input->post('name', TRUE) . "' تم حفظها بنجاح", base_url() . "judgeshead/categories/home");
    ...
    function load_subcategories() {
        ...
        if (isset($id) && $id != "") {
            ...
            $this->load->view("frontend/superjudge/category/ajax/view_subcategoies", $data);
        }
    }
    function add_subcategory() 
    {
        ...
        if(isset($data['cat_id']) && $data['cat_id'] != ""){
            ...
            $this->load->view('frontend/superjudge/category/ajax/add_subcategory', $data);
        }
    }
